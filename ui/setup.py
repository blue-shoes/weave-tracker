...
class Stage2():
    ...
    def open(self):
        ...
        self.hardware.place_text("Set Levers", 0.75, int(self.hardware.WIDTH/2), 20)
        width = max(self.hardware.display.measure_text("Back", scale=0.5), self.hardware.display.measure_text("Next", scale=0.5), self.hardware.display.measure_text("Down", scale=0.5))
        button_render.place_button(self.hardware, "Back", width, self.hardware.BTN_HEIGHT, TOP_LEFT)
        button_render.place_button(self.hardware, "Next", width, self.hardware.BTN_HEIGHT, TOP_RIGHT)
