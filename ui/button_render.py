...
def place_button(hardware, text, width, height, button_placement, width_pad=2, height_pad=2):
    if button_placement == TOP_LEFT:
        hardware.display.text(text, int(width/2) + width_pad, hardware.HEIGHT - int(height/2 + height_pad))
        hardware.display.line(0, hardware.HEIGHT-(height + 2*height_pad), (width + 2*width_pad), hardware.HEIGHT-(height + 2*height_pad))
        ...
        hardware.display.text(text, hardware.WIDTH - int(width/2 + width_pad), hardware.HEIGHT - int(height/2 + height_pad))
        ...
        hardware.display.text(text, int(width/2) + width_pad, int(height/2 + height_pad))
        hardware.display.line(0, (height + 2*height_pad), (width + 2*width_pad), (height + 2*height_pad))
        hardware.display.line(width + 2*width_pad, 0, width + 2(width_pad), (height + 2*height_pad))
    elif button_placement == BOTTOM_RIGHT:
        hardware.display.text(text, hardware.WIDTH - int(width/2 + width_pad), int(height/2 + height_pad))
